...
共识是指这些特性和工作将来确定会发生，以及对于用户来说大致是什么样的。
...
- **MVP:** 您可以了解一下这个功能，也许在将来对您有所帮助。
- **Beta:** 您可以从中受益，但是您在使用之前应该仔细评估该功能。
- **Ready and Evolving:** 生产可用，但是请注意，将来在升级Flink时，可能需要对你的应用和设置进行一些调整。
- **Stable:** 可以在生产中稳定不受限制地使用。
- **Reaching End-of-Life:** 仍然可以稳定使用，但请考虑替代方法。对于新的长期项目而言，这不是很好的选择。
- **Deprecated:** 不推荐使用，您需要开始寻找替代品

<hr />


# 一体化分析：流批一体，SQL及其他
Flink的内核是流数据处理系统，他把批作为流的特例，用流的方式来执行批处理。
作为一个流式引擎，Flink不仅能够高效执行批处理，更重要的是他通过高效处理有限流的方式
打开了无缝流批一体处理之门。

流批一体升级了流数据范例：它可以保证实时和离线应用的语义一致性。
此外，有时流式处理的作业也需要离线（有限流）处理作为补充，例如，
在出现错误或数据质量问题时需要重新处理数据，
或者启动新的作业但需要历史数据作为引导。统一的API和系统使得此类操作变得很容易。

## 统一的SQL平台

Flink社区一直致力于建设基于Flink的统一的流批一体SQL分析平台，并将持续在这个方向上努力。
SQL具有非常强的跨流批的语义，并允许用户使用相同的SQL语句对即时查询（ad-hoc query）和
连续查询（continuous query）进行分析。Flink拥有高效的统一查询引擎，以及基于此的一系列
整合统一。根据用户反馈，这些整合统一使用体验会持续改善。
...
**CDC & Connectors**
  ...
  - 数据湖Connectors: 流批一体对数据湖有很大价值。支持流（处理当前数据）和批（处理历史数据）的相同API，相同语义，以及相同引擎。
...
*DataStream API* 是 Flink 的*物理层* API, 针对需要明确地控制数据类型，数据流，状态以及时间的用例。
...
**DataStream 的批处理执行模式**
  ...
  - Flink 在 DataStream 上为有限流新增加了*批执行模式*，这可以使得用户更简单快速的执行和恢复有限流作业。 
    ...
    值得注意的是，此模式的实现基于上面提到的新的的 source 和 sink 的实现，因此它只能支持已经使用新 API 的 connectors。
...
- 在Table API中使 Source 和 Sink 更容易定义使用                                                  

- DataStream API & Table APIs 互通性: 
...
我们希望部署一个流式作业就像启动一个独立的应用（Applications）一样简单；而不需要
首先启动集群（Clusters），再向该集群提交作业。
...
Flink的容错机制多年来运行非常稳定，但是我们还是想让整个容错过程更快，更可预测，并消除在某些方面的困惑提升易用性。
