...
These data streams can be streams of real-time data, or stored streams of historic data.
For example, in Flink's view a File is also a stored stream of bytes. Because of that, Flink
supports both real-time data processing and applications, as well batch-processing applications.

Streams can be *unbounded* (have no end, events continuously keep coming) or be *bounded* (streams have a beginning
and an end). For example, a Twitter feed or a stream of events from a message queue is generally an unbounded stream,
file a stream of bytes from a file is a bounded stream.

## If everything is a stream, why is there a DataStream and DataSet API in Flink?
...
results (often with low latency).

Processing bounded streams usually does not require to produce low latency results, because the data is a while old
anyways (in relative terms). That allows Flink to process the data in a simple and more efficient way.
...
The *DataSet* API has techniques that often speed up the processing of bounded data streams. In the future, the project
wants to combine these optimizations with the techniques in the DataStream API.
...
When running together with these components, Flink can use HDFS to read data, or write results and snapshots.
...
[Docker](https://www.docker.com/), or even standalone services.

## What are the prerequisites to use Flink?

  - You need **Java 8** to run the Flink jobs/applications.
  - For the Scala API (optional), Scala 2.11
  - Highly-available setups with no single point of failure require [Apache ZooKeeper](https://zookeeper.apache.org/)

  - For highly-available stream processing setups, Flink requires some form of distributed storage for checkpoints (HDFS / S3 / NFS / SAN / GFS / Kosmos / Ceph / ...)
...
For the DataStream API, Flink supports larger-than-memory state using the RocksDB state backend.

For the DataSet API, all configurations can scale beyond main memory.
...
Such errors typically indicate a mixup in some dependency version. That means a different version of a dependency (a library)
