...
The Flink documentation is maintained in **US English** and **Chinese** — when extending or updating the documentation, both versions should be addressed in one pull request. If you're not familiar with the Chinese language, make sure that your contribution is complemented by these additional steps:

* Open a [JIRA](https://issues.apache.org/jira/projects/FLINK/issues/FLINK-12070?filter=allopenissues) ticket for the translation, tagged with the _chinese-translation_ component;
...
* **Use the active voice.** The [active voice](https://medium.com/@DaphneWatson/technical-writing-active-vs-passive-voice-485dfaa4e498) supports brevity and makes content more engaging. If you add _by zombies_ after the verb in a sentence and it still makes sense, you are using the passive voice.
...
* **Use _you_, never _we_.** Using _we_ can be confusing and patronizing to some users, giving the impression that “we are all members of a secret club and _you_ didn’t get a membership invite”. Address the user as _you_.
...
Use clear definitions of terms or provide additional instructions on what something means by adding a link to a helpful resource, such as other documentation pages or the [Flink Glossary](https://ci.apache.org/projects/flink/flink-docs-master/concepts/glossary.html). The Glossary is a work in progress, so you can also contribute and propose new terms in [FLINK-13002](https://jira.apache.org/jira/browse/FLINK-13002?jql=project%20%3D%20FLINK%20AND%20text%20~%20glossary).
...
<div class="alert alert-warning">
  ...
  For every documentation file, the front matter should be immediately followed by the Apache License statement. For both language versions, this block must be stated in US English and copied in the exact same words as the following example.
</div>
...
<div class="alert alert-warning">
  <h3>Best Practice</h3>
  Use descriptive link texts that provide information on the action or destination. For example, avoid using "Learn More" or "Click Here".
</div>
...
* **Code blocks.** Code that represents self-contained examples, feature walkthroughs, demonstration of best practices or other useful scenarios should be wrapped using a fenced code block with appropriate [syntax highlighting](https://github.com/rouge-ruby/rouge/wiki/List-of-supported-languages-and-lexers). One way of achieving this with markup is:
  ...
  * **Showcase key development tasks.** Reserve code examples for common implementation scenarios that are meaningful to users. Leave more lengthy and complicated examples for tutorials or walkthroughs.
...
The Flink community is diverse and international, so you need to think wide and globally when writing documentation. Not everyone speaks English at a native level and the level of experience with Flink (and stream processing in general) ranges from absolute beginners to experienced power users. Ensure technical accuracy and linguistic clarity in the content you produce so that it can be understood by all users.
