...
stop a job with savepoint that is consistent with the emitted data.
...
processor. The Blink query processor offers better SQL coverage (full TPC-H
...
However, the integration of the Blink’s query processor is not fully completed
...
dependency](https://ci.apache.org/projects/flink/flink-docs-master/dev/table/#table-program-dependencies)
to your project.
...
on a whole other set of improvements to these interfaces, including:
 ...
 * **Rework of the Table API Type System**
   **([FLIP-37](https://cwiki.apache.org/confluence/display/FLINK/FLIP-37%3A+Rework+of+the+Table+API+Type+System))**
   ...
   system](https://ci.apache.org/projects/flink/flink-docs-master/dev/table/types.html#data-types)
   to detach the Table API from Flink’s
   [TypeInformation](https://ci.apache.org/projects/flink/flink-docs-release-1.8/dev/types_serialization.html#flinks-typeinformation-class)
   class and improve its compliance with the SQL standard. This is still a
   work in progress and expected to be completed for the next release. In
...
be able to query and process all data that is stored in Hive. The Hive
...
volatile, the new catalog connector also allows to persist a table in
...
This release also introduces a first version
[FLIP-38](https://cwiki.apache.org/confluence/display/FLINK/FLIP-38%3A+Python+Table+API):
A Python Table API. This marks the start towards our goal of bringing
...
PyFlink](https://ci.apache.org/projects/flink/flink-docs-master/flinkDev/building.html#build-pyflink).
From there you can have a look at [this
walkthrough](https://ci.apache.org/projects/flink/flink-docs-master/tutorials/python_table_api.html)
...
Driesprong, Gao Yun, Gary Yao, Gen Luo, GitHub, Gyula Fora, Hequn Cheng,
...
Jingsong Lee, JingsongLi, Juan Gentile, Jungtaek Lim, Kailash Dayanand, Kevin
...
Timo Walther, TisonKun, Tom Goong, TsReaper, Tzu-Li (Gordon) Tai, Ufuk Celebi,
...
Tang, Zhanchun Zhang, Zhenghua Gao, Zhijiang, Zhu Zhu, Zi Li, ZiLi Chen, Zili
Chen, aloys, ap, arganzheng, azagrebin, bd2019us, beyond1920, biao.liub,
blueszheng, boshu Zheng, bowen.li, chenqi, chummyhe89, chunpinghe, dcadmin,
dianfu, godfrey he, godfreyhe, guanghui01.rong, hehuiyuan, hello, hequn8128,
ifndef-SleePy, jackyyin, joongkeun.yang, klion26, lamber-ken, leesf, liguowei,
...
shenlang.sl, shuai-xu, sunhaibotb, sunjincheng121, tianchen, tianchen92,
tison, tom_gong, vinoyang, vthinkxie, wanggeng3, wenhuitang, winifredtamg,
xl38154, xuefuz, xuyang1706, yangfei5, yanghua, yuzhao.cyz, zentol,
zhangxin516, zhangxinxing, zhaofaxian, zhijiang, zhuzhu.zz, zjuwangg, 云邪, 林小铂,
黄培松, 姬平, 时无两丶.
