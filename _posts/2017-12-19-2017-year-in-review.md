...
Work is already underway on a number of these features, and some will even be included in Flink 1.5 at the beginning of 2018.
...
-   **Fine-grained recovery from task failures**, as described in [FLIP-1](https://cwiki.apache.org/confluence/display/FLINK/FLIP-1+%3A+Fine+Grained+Recovery+from+Task+Failures) to improve recovery efficiency and only re-execute failed tasks, reducing the amount of state that needs to be transferred on recovery.
...
Lastly, we'd like to extend a sincere thank you to all of the Flink community for making 2017 a great year!
