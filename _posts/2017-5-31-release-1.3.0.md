...
* **Incremental Checkpointing for RocksDB**: It is now possible to checkpoint only the difference from the previous successful checkpoint, rather than checkpointing the entire application state ([FLINK-5053](https://issues.apache.org/jira/browse/FLINK-5053))
...
The CEP library has been greatly enhanced and is now able to accommodate more use-cases out-of-the-box (expressivity enhancements), make more efficient use of the available resources, adjust to changing runtime conditions--all without breaking backwards compatibility. 
...
* Add Evenly Graph Generator to Flink Gelly ([FLINK-6393](https://issues.apache.org/jira/browse/FLINK-6393))
