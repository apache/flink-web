...
The Apache Flink community is excited to hit the double digits and announce the release of Flink 1.10.0! As a result of the biggest community effort to date, with over 1.2k issues implemented and 218 contributors, this release introduces significant improvements to the overall performance and stability of Flink jobs, a preview of native Kubernetes integration and great advances in Python support (PyFlink). 
...
The binary distribution and source artifacts are now available on the updated [Downloads page]({{ site.baseurl }}/downloads.html) of the Flink website. For more details, check the complete [release changelog]() and the [updated documentation]({{ site.DOCS_BASE_URL }}flink-docs-release-1.10/). We encourage you to download the release and share your feedback with the community through the [Flink mailing lists](https://flink.apache.org/community.html#mailing-lists) or [JIRA](https://issues.apache.org/jira/projects/FLINK/summary).
...
To make memory options more explicit and intuitive to users, Flink 1.10 introduced significant changes to the `TaskExecutor` memory model and configuration logic ([FLIP-49](https://cwiki.apache.org/confluence/display/FLINK/FLIP-49%3A+Unified+Memory+Configuration+for+TaskExecutors)). These changes make Flink more adaptable to all kinds of deployment environments (e.g. Kubernetes, Yarn, Mesos), giving users strict control over its memory consumption.

**Out-of-the-box Managed Memory Configuration**

Managed memory was extended to also account for memory usage of state backends. This means that there may be scenarios where jobs in the same cluster require different types of memory (e.g. a streaming job with `MemoryStateBackend` and another streaming job with `RocksDBStateBackend`). To enable these scenarios and allow users to switch between Streaming and Batch execution without having to modify cluster configurations, managed memory is now always off-heap.
...
<span class="label label-danger">Note</span> FLIP-49 changes the process of cluster resource configuration, which may require tuning your clusters for upgrades from previous Flink versions. For a comprehensive overview of the changes introduced and tuning guidance, consult [this setup]({{ site.DOCS_BASE_URL }}flink-docs-release-1.10/ops/mem_setup.html).
...
<span>
	<center>
	<img vspace="8" style="width:100%" src="{{site.baseurl}}/img/blog/2020-02-03-release-1.10.0/flink_1.10_zeppelin.png" />
	</center>
</span>
...
<span>
	<center>
	<img vspace="8" style="width:75%" src="{{site.baseurl}}/img/blog/2020-02-03-release-1.10.0/flink_1.10_nativek8s.png"/>
	</center>
</span>
...
Hive integration was announced as a preview feature in Flink 1.9. This first implementation allowed users to persist Flink-specific metadata in Hive Metastore using SQL DDL, call UDFs defined in Hive and use Flink for reading and writing Hive tables. However, some further development was still required to really bring production-ready Hive integration to Flink.


#### Partition Support for Batch SQL
...
Besides partition pruning, Flink 1.10 introduces more optimizations to Hive integration, such as:

* **Projection pushdown:** Flink leverages projection pushdown to minimize data transfer between Flink and Hive tables by omitting unnecessary fields from table scans. It is especially beneficial when a table contains many columns.
...
In order to correctly handle and guarantee a consistent behavior across meta-object (tables, views, functions) in the future, some object declaration methods in the Table API  have been deprecated in favor of methods that are closer to standard SQL DDL ([FLIP-64](https://cwiki.apache.org/confluence/display/FLINK/FLIP-64%3A+Support+for+Temporary+Objects+in+Table+module)).
...
<span>
	<center>
	<img vspace="8" hspace="100" style="width:75%" src="{{site.baseurl}}/img/blog/2020-02-03-release-1.10.0/flink_1.10_pyflink.gif"/>
	</center>
</span>
 ...
 * [[FLINK-14516](https://issues.apache.org/jira/browse/FLINK-13884)] The non-credit-based network flow control code was removed, along with the configuration option `taskmanager.network.credit.model`. Moving forward, Flink will always use credit-based flow control.
