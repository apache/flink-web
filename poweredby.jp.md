...
Apache Flink は世界中の多くの企業でビジネスに不可欠なアプリケーションを動かしています。このページでは、本番環境で興味深いユースケースを実行している注目すべきFlinkユーザを紹介し、それらのアプリケーションについて詳しく説明しているリソースにリンクしています。

より多くの Flink ユーザーはプロジェクト wiki の <a href="https://cwiki.apache.org/confluence/display/FLINK/Powered+by+Flink" target='_blank'><small><span class="glyphicon glyphicon-new-window"></span></small> Powered by Flink directory</a> にリストアップされています。このリストは *包括的ではない* ことに注意してください。私たちは、明示的にリストに載せることを要求したユーザのみを追加します。

このページへの掲載をご希望の方は、[Flinkユーザーメーリングリスト]({{ site.baseurl }}/community.html#mailing-list)までご連絡ください。
